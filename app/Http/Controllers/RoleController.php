...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('out_of_shift')){
                $permission = Permission::firstOrCreate(['name' => 'out_of_shift']);
                if(!$role->hasPermissionTo('out_of_shift')){
                    $role->givePermissionTo($permission);
                }
        }
        else
            $role->revokePermissionTo('out_of_shift');
        ...
        if($request->has('zkteco_configuration')){
            $permission = Permission::firstOrCreate(['name' => 'zkteco_configuration']);
            if(!$role->hasPermissionTo('zkteco_configuration')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('zkteco_configuration');
